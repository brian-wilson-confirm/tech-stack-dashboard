...
type Props<T extends Record<string, any>> = {
  ...
  editingRow: string | null | undefined
  onSaveEdit?: () => void
  onCancelEdit?: () => void
...
export function DataTableComponent<T extends Record<string, any>>({
  ...
  const table = useReactTable({
    ...
    pageCount: showPagination && pagination ? Math.ceil((data?.length ?? 0) / pagination.pageSize) : undefined,
    filterFns: {
  ...
  const paginationState = pagination ?? table.getState().pagination ?? { pageIndex: 0, pageSize: (data?.length ?? 0) };
  const currentPageIndex = paginationState.pageIndex;
  const pageSize = paginationState.pageSize;
  const page = currentPageIndex + 1;
  const totalPages = pageSize > 0 ? Math.ceil((data?.length ?? 0) / pageSize) : 1;
    ...
    <div className="border rounded-lg p-6 col-span-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex flex-1 items-center space-x-2">
          ...
          <div className="relative">
            ...
            {searchQuery !== undefined && setSearchQuery && filterConfigs?.map((config) => (
              <FilterDropdown key={config.field} config={config} editingRow={editingRow ?? null} />
            ))}
        ...
        <div className="rounded-md border overflow-x-auto">
          ...
          <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={(columns?.length ?? 0) + (showCheckboxes ? 1 : 0) + (showActions ? 1 : 0)} className="h-24 text-center">
                    Loading...
                  </TableCell>
                </TableRow>
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="text-sm text-muted-foreground">
              ...
              {(data?.length ?? 0)} {(data?.length ?? 0) === 1 ? "row" : "rows"} available
        </div>
