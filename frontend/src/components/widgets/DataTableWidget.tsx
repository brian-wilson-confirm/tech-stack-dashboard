...
export function DataTableWidget({ rows: initialRows }: DataTableWidgetProps) {
  //console.log('(NewWidget) Initial tasks in widget:', initialRows)
  ...
  const sortRows = (rowsToSort: Task[]) => {
    if (sortConfigs.length === 0) return rowsToSort
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((rowData) => {
      const matchesSearch = rowData.task.toLowerCase().includes(searchQuery.toLowerCase())
      const matchesStatus = selectedStatus.length === 0 || selectedStatus.includes(rowData.status)
      const matchesPriority = selectedPriority.length === 0 || selectedPriority.includes(rowData.priority)
      return matchesSearch && matchesStatus && matchesPriority
    })
  }, [rows, searchQuery, selectedStatus, selectedPriority])

  const totalPages = Math.ceil(filteredRows.length / rowsPerPage)
  const start = (page - 1) * rowsPerPage
  const end = start + rowsPerPage
  const sortedRows = sortRows(filteredRows)
  const currentRows = sortedRows.slice(start, end)
  ...
  const saveEditing = async () => {
    if (editForm) {
      try {
        ...
        toast({
          title: "Error",
          //description: error instanceof Error ? error.message : "Failed to update the row. Please try again.",
          description: "Failed to update the row. Please try again.",
          variant: "destructive",
          duration: 3000,
        });
      }
  ...
  const addRow = async (newRow: TaskFormValues) => {
    try {
      const response = await fetch('http://localhost:8000/api/tasks', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        const errorData = await response.json()
        throw new Error(errorData.detail || 'Failed to add row')
      }
              ...
              <TableHead className="w-12">
                <Checkbox
                  ...
                  onCheckedChange={(checked: boolean) => {
                    setSelectedRows(checked ? currentRows.map(t => t.id) : [])
                  }}
              ...
              <TableRow key={rowData.id}>
                <TableCell>
                  <Checkbox
                    ...
                    aria-label={`Select task ${rowData.task}`}
                  />
                </TableCell>
                {columnVisibility["task_id"] && (
                  <TableCell>
                ...
                {columnVisibility["status"] && (
                  <TableCell>
                    {editingRow === rowData.id ? (
                      ...
                      <Badge variant="secondary" className={`${getStatusColor(rowData.status)} text-white`}>
                ...
                {columnVisibility["priority"] && (
                  <TableCell>
                    {editingRow === rowData.id ? (
                      ...
                      <Badge variant="secondary" className={`${getPriorityColor(rowData.priority)} text-white`}>
                        {rowData.priority}
                      </Badge>
                    )}
                  </TableCell>
                  ...
                  <TableCell>
                    {editingRow === rowData.id ? (
                      ...
                      <div className="flex items-center gap-2">
                        <Progress value={rowData.progress} className="w-[60px]" />
                        <span className="text-sm">{rowData.progress}%</span>
                      </div>
                    )}
                  </TableCell>
                )}
                {columnVisibility["order"] && <TableCell>{rowData.order}</TableCell>}
                {columnVisibility["actual_duration"] && (
                  <TableCell>
                    {editingRow === rowData.id ? (
                      <Input
                  ...
                  <TableCell>
                    {editingRow === rowData.id ? (
                      <Input
                        ...
                        className="w-[130px]"
                      />
                    ) : rowData.start_date ? new Date(rowData.start_date).toLocaleDateString() : '-'}
                  </TableCell>
                )}
                {columnVisibility["end_date"] && (
                  <TableCell>
                    {editingRow === rowData.id ? (
                      <Input
                        ...
                        className="w-[130px]"
                      />
                    ) : rowData.end_date ? new Date(rowData.end_date).toLocaleDateString() : '-'}
                  </TableCell>
                )}
                {columnVisibility["done"] && (
                  <TableCell>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    {editingRow === rowData.id ? (
                      ...
                      <>
                        <Button variant="ghost" size="icon" onClick={() => startEditing(rowData)}>
